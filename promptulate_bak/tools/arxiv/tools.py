...
class ArxivReferenceTool(Tool):
    ...
    def _run(self, query: str, *args, **kwargs) -> Union[str, List[Dict]]:
        ...
        prompt = (
            f"Based on the specific content of the research, you need to list at least {self.max_reference_num} reference papers. You can use arxiv for the query. You need to provide me with 3 arxiv query keywords. I will use arxiv for the query, and you need to select the most relevant {self.max_reference_num} reference papers based on the results I return."  # noqa
            f"User input: {query}"
            "Your output must be three query words\n, such as [query]: keyword1, keyword2, keyword3\n"  # noqa
        )
