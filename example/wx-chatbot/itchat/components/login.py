    ...
    while self.isLogging:
        ...
        if uuid:
            ...
            qrStorage = self.get_QR(
                enableCmdQR=enableCmdQR, picDir=picDir, qrCallback=qrCallback
            )
...
def process_login_info(core, loginContent):
    ...
    * skey, wxsid, wxuin, pass_ticket will be fetched
    ...
    if not all(
        [key in core.loginInfo for key in ("skey", "wxsid", "wxuin", "pass_ticket")]
    ):
        logger.error(
        ...
        while self.alive:
            try:
                ...
                if self.receivingRetryCount < retryCount:
                    logger.error(
                        "Having tried %s times, but still failed. " % (retryCount)
                        + "Stop trying..."
                    )
                    self.alive = False
                else:
...
def get_msg(self):
    ...
    self.loginInfo["SyncKey"] = dic["SyncKey"]
    self.loginInfo["synckey"] = "|".join(
        ["%s_%s" % (item["Key"], item["Val"]) for item in dic["SyncCheckKey"]["List"]]
    )
