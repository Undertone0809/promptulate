    ...
    * 53 webwxvoipnotifymsg, 9999 sysnotice
    ...
    for m in msgList:
        ...
        if "@@" in actualOpposite:
            m["User"] = core.search_chatrooms(
                userName=actualOpposite
            ) or templates.Chatroom({"UserName": actualOpposite})
            ...
            m["User"] = (
                core.search_mps(userName=actualOpposite)
                or core.search_friends(userName=actualOpposite)
                or templates.User(userName=actualOpposite)
            )
        ...
        if m["MsgType"] == 1:  # words
            ...
            download_fn = get_download_fn(
                core, "%s/webwxgetvoice" % core.loginInfo["url"], m["NewMsgId"]
            )
            msg = {
                "Type": "Recording",
...
def upload_chunk_file(
    core, fileDir, fileSymbol, fileSize, file_, chunk, chunks, uploadMediaRequest
):
    url = (
        core.loginInfo.get("fileUrl", core.loginInfo["url"])
    ...
    files = OrderedDict(
        [
            ...
            (
                "lastModifiedDate",
                (None, time.strftime("%a %b %d %Y %H:%M:%S GMT+0800 (CST)")),
            ),
