...
class Chatroom(AbstractUserDict):
    def __init__(self, *args, **kwargs):
        ...
        def init_fn(parentList, d):
            d.chatroom = refSelf() or parentList.core.search_chatrooms(
                userName=userName
            )
...
class ChatroomMember(AbstractUserDict):
    ...
    def get_head_image(self, imageDir=None):
        return self.core.get_head_img(
            self.userName, self.chatroom.userName, picDir=imageDir
        )

    def delete_member(self, userName):
        return self.core.delete_member_from_chatroom(
            self.chatroom.userName, self.userName
        )
